<script lang="ts">
	...
	// 	const response = await enhance(formData, {
	// 		method: 'POST',
	// 		action: '?/createProduct'
	// 	});

	// 	if (response.success) {
	// 		toast.success('Το προϊόν δημιουργήθηκε με επιτυχία');
	// 		onSuccess();
	// 		isDialogOpen.set(false);
	// 	} else {
	// 		toast.error('Αποτυχία δημιουργίας προϊόντος');
	// 	}
	// }
</script>
...
<form method="POST" action="?/createProduct" use:enhance>
	...
	<Dialog.Footer>
		<Button variant="secondary" type="button" on:click={() => isDialogOpen.set(false)}>
			Ακύρωση
		</Button>
		<!-- TODO close and refresh upon successful submission -->
